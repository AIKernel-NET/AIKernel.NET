...
## 1. Purpose
Define the capability-vector selection contract for `IModelVectorRouter`.
...
- `MRS-F004` Dynamic Limit Violation: If `IDynamicCapacityProvider` detects overload or unstable connectivity immediately before execution, force fallback or abort instead of degraded forced execution.
...
- AC4: Overload/connectivity degradation transitions to fallback or abort.

## 6. ModelCapacityVector Structure (ROM Representation)
...
3. Soft Scoring  
   `IVectorMatcher` computes cosine similarity or weighted distance.
4. Dynamic Adjustment  
   Apply budget/load penalties from `IDynamicCapacityProvider`.
5. Final Selection  
   Select highest score and record traceability matrix into Replay Dump.
