...
- `SGS-F005` Trusted Anchor Unreachability: If `ISignatureTrustStore` query times out or fails, treat as `Indeterminate` and block execution.

## 5. Acceptance Criteria
- AC1: Unsigned artifacts are non-executable.
- AC2: Revoked signer artifacts are non-executable.
- AC3: Execution is allowed only with valid signature and scope.
- AC4: Expired artifacts are rejected with second-level precision.
- AC5: TrustStore unreachability always results in `Deny`.

## 6. Artifact Anatomy (ROM/YAML Structure)
...
2. Integrity Check  
   Recompute hash from ROM body and metadata, then compare to `hash`.
...
5. Policy Authorization  
   Use `IPromptValidator` to validate policy against runtime constraints.
6. Unfreezing  
