...
Wasm integration should keep browser-specific behavior out of Core contracts.

### JA

Wasm Integration は 0.1.2 の repository metadata、package metadata、既存 docs を根拠に説明します。
...
| [`aikernel-wasm`](/reference/aikernel-wasm/python/aikernel-wasm/) | python | `0.1.2` | Python wrapper for AIKernel.Wasm 0.1.2 runtime and WebGPU provider assemblies. | `AIKernel.Wasm/python/pyproject.toml` |
