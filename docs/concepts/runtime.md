...
This page explains runtime using repository metadata and links to the generated Reference.

### JA

このページでは Runtime を repository metadata と generated Reference に基づいて説明します。
...
`AIKernel.Core`, `AIKernel.Control`, and `AIKernel.Wasm` together expose runtime, governance, and browser isolation surfaces.
...
- Runtime は package family をまたぐため、単一の class ではなく flow と boundary で理解します。
