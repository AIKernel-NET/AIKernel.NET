...
This page explains tools using repository metadata and links to the generated Reference.

### JA

このページでは Tools を repository metadata と generated Reference に基づいて説明します。
...
`AIKernel.Tools` contains 6 NuGet packages and public namespaces such as `AIKernel.CLI, AIKernel.CLI.Commands, AIKernel.CLI.Services, AIKernel.Tools.Capability.RomStorage`.
...
- Tool は runtime を動かす本体ではなく、観測、検査、再生、運用を助ける面です。
