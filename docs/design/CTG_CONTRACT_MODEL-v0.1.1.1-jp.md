...
`DecisionGateRequest` は council evaluation carrier より意図的に狭い。
これは 1 つの `GateInput` だけを運び、個別の `L` / `E` / `P` `GateInput`
envelope や `CouncilDecision` payload は運ばない。`GateInput` は純粋な
triadic vote-only carrier である。

- `Logos`
- `Ethos`
- `Pathos`

この 3 値は `CouncilVoteValue` で表現する。confidence、risk、source、
metadata の observation は council trace または result diagnostics に属し、
decision gate input には属さない。
...
| --- | --- | --- |
| Council vote carrier | `CouncilVote` | 1 件の council vote と trace evidence を運ぶ。 |
| Council vote enum | `CouncilVoteValue` | finite vote vocabulary を carrier DTO から分離する。 |
...
`CouncilVoteValue` の C# enum value はその weight ではなく、安定した
...
- Ethos rejection は execution を deny する。
- Ethos-veto case は 4 つ目の vote value ではなく rejection evidence として表現する。
- Ethos denial がない場合、map された aggregate council vote が strictly positive
  の場合のみ execution を admit する。
- zero、negative、missing、unknown、insufficient evidence は execution を deny する。

gate / trajectory decision enum は discrete-only である。
...
ROM/YAML source が uppercase snake case を使う場合でも、`RejectReasonKind` は
PascalCase C# name を使う。
...
正規化された `CanonReference` shape は次の通りである。
...
- 既存 public interface signature を変更しない。
- DTO または enum の正規化は compatibility impact とともに文書化する。
