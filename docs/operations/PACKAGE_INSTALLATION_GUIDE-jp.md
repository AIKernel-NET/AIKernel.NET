...
| ライン | 意味 |
|---|---|
| `0.1.2` | 最初に同期された public Semantic OS package line。 |
| `0.1.2` | NuGet package と同期 Python wrapper を揃えた public canonical package line。 |
| `0.1.2` | 現在の正典シリーズターゲット。公開開始後は NuGet / PyPI package family を同期。 |

0.1.2 時点の CTG ドキュメントは migration context として維持します。
...
behavior を明示的に試験する場合を除き、release validation environment で `0.1.2`、
`0.1.2`、`0.1.2` を混在させないでください。
