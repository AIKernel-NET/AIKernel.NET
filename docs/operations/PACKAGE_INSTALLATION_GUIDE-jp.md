...
AIKernel 0.1.2 は、Semantic OS サーフェスの次期正典パッケージシリーズです。
NuGet パッケージと Python ラッパーパッケージを同じ public API catalog に揃えつつ、
実装ロジックは managed package 側に維持します。

安定版 `0.1.2` パッケージは、公開タスクが明示的に開始されるまで作成しません。
...
[`../development/package-release-alignment-v0.1.2-ja.md`](../development/package-release-alignment-v0.1.2-ja.md)
にまとめています。RC5 monolith 由来の CTG-ROM は、本文・ルール・意味論を変更せず、
metadata のみを正典 `0.1.2` に揃えています。詳細は
[`CTG_ROM_LAYOUT-v0.1.2-jp.md`](CTG_ROM_LAYOUT-v0.1.2-jp.md)
を参照してください。

## Quick Start: 正典 0.1.2

安定版公開後は、同じ `0.1.2` ラインの package をレイヤーごとに導入します。
...
ローカル検証では、安定版 version を `0.1.2-dev<build-number>` に置き換え、
...
0.1.2 line では、Python package は managed package API catalog に対する薄い wrapper
...
ローカル検証では `0.1.2.dev<build-number>` の local wheel を使い、Python wrapper の
...
| ライン | 意味 |
|---|---|
| `0.1.1` | 最初に同期された public Semantic OS package line。 |
| `0.1.1.1` | NuGet-only の additive interface-extension update。Python wrapper は `0.1.1` のまま維持。 |
| `0.1.2` | 現在の正典シリーズターゲット。公開開始後は NuGet / PyPI package family を同期。 |

0.1.1.1 時点の CTG ドキュメントは migration context として維持します。
...
正典 line では、同じ `0.1.2` package family だけを組み合わせてください。migration
behavior を明示的に試験する場合を除き、release validation environment で `0.1.1`、
`0.1.1.1`、`0.1.2` を混在させないでください。
