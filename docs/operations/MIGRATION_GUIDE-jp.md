...
### 15.6 DynamicSLM / SeedSLM / HATL / governance vocabulary contract 準備
v0.0.5 では、将来の DynamicSLM capability module、SeedSLM discipline surface、HATL external cryptographic operator、pre-inference governance admission evidence 向けに runtime を持たない contract を追加します。
既存 consumer には source-compatible な追加ですが、capability-modular SLM artifact、HATL-backed trust layer、Semantic DSL admission evidence を扱う Core / Provider 実装は新しい namespace を対象にしてください。
...
| `AIKernel.Dtos.Governance` | replay-compatible な pre-inference admission evidence を運ぶ `AdmissibilityReplayRecord` |
...
- v0.0.5 (2026-06-05): Abstractions-local DTO/例外実装、DTO enum 重複、旧 ChatChain 曖昧 interface を削除し、DynamicSLM Model ABI / SeedSLM discipline / distillation offload / HATL external cryptographic operator / governance admissibility / Semantic IR vocabulary contract 準備を追加
