...
| `AIKernel.Enums` | DynamicSLM payload、accelerator、distillation offload / fallback selection を含む pipeline stage、failure kind、capability relation、compatibility status、graph update、admission status、distillation job status、fallback kind、pipeline status primitive |
