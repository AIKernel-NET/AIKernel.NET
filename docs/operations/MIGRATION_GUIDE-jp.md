...
本ガイドは、初期コンセプト版（v0.0.0）から、正典化されたアーキテクチャ（v0.0.1、v0.0.2、v0.0.3）、v0.0.4 の DSL / History ROM contract 抽出、および v0.0.5 の contract-surface purity cleanup と DynamicSLM contract 準備へ移行するための手順を定義します。
...
### 15.6 DynamicSLM Model ABI contract 準備
...
|---|---|
| `AIKernel.Abstractions.DynamicSlm` | `IDynamicSlmModelAbiProvider`, `IDynamicSlmModuleRegistry`, `IDynamicSlmCapabilityGraphResolver`, `IDynamicSlmLineageVerifier`, `IDynamicSlmPayloadLoader`, `IDynamicSlmScheduler`, `IDynamicSlmCapabilityGapDetector`, `IDynamicSlmDistillationPlanner`, `IDynamicSlmArtifactPublisher` |
| `AIKernel.Dtos.DynamicSlm` | semantic profile、capability graph、execution profile、lineage、payload descriptor、resolved subgraph、placement plan、capability gap、distillation plan、admission result の Model ABI record |
| `AIKernel.Enums` | DynamicSLM payload、accelerator、capability relation、admission status primitive |
...
- v0.0.5 (2026-06-05): Abstractions-local DTO/例外実装、DTO enum 重複、旧 ChatChain 曖昧 interface を削除し、DynamicSLM Model ABI contract 準備を追加
