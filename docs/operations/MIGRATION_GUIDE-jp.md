...
`AIKernel.Abstractions.Execution.IKernelExecutor` と `AIKernel.Abstractions.Rom.ISemanticHasher` は変更しません。改名対象は、facade / chat-chain 専用でありながら、実行境界や ROM 概念と名前衝突していた contract です。
