...
| `AIKernel.Dtos.DynamicSlm` | semantic profile、capability graph、execution profile、lineage、payload descriptor、pipeline context/result/failure/trace metadata、resolved subgraph、placement plan、capability gap、graph update plan、distillation plan、admission result の Model ABI record |
| `AIKernel.Enums` | DynamicSLM payload、accelerator、pipeline stage、failure kind、capability relation、compatibility status、graph update、admission status primitive |
