...
v0.0.5 adds non-runtime contracts for future DynamicSLM capability modules, SeedSLM discipline surfaces, HATL external cryptographic operators, pre-inference governance admission evidence, trajectory governance evidence, and Semantic Compilation descriptors.
...
| `AIKernel.Dtos.Governance` | `AdmissibilityReplayRecord` for replay-compatible pre-inference admission evidence; `SemanticEllipsoidDescriptor`, `TrajectoryGovernanceScoreReport`, and `CandidateActionEvaluation` for trajectory governance evidence |
