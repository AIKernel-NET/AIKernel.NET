...
- 0.1.1.1 ラインは local validation line であり、NuGet-only です。
- 0.1.1.1 における Python wrapper 文書は参照資料です。0.1.1.1 検証のために
  PyPI package を build / publish / require しません。
- 次の公式 release line は v0.1.2 正典シリーズです。この line では、NuGet と PyPI の
  package family が同期して公開される前提で準備します。
