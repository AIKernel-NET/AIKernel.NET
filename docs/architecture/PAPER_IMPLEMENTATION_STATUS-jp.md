...
| 12 | Canonical Trajectory Governance (CTG) | **Contracted for v0.1.1.1**。CTG council / gate / trajectory / ROM governance interface、`CouncilKind`、`CouncilVoteKind`、`CouncilDecisionKind`、gate decision enum、`CanonReference`、`RejectReasonInfo`、`CouncilVoteValue`、`CouncilDecision`、`DecisionGateResult`、`StepGovernanceTrace`、`GovernanceTrace`、ROM governance carrier に反映済み。実装は論文 sketch より詳細だが、論文 invariant を維持する。 | finite vote-table execution、Ethos-denial short-circuit、CTG-ROM loading、persona profile resolution、PPM research integration、HATL-backed state/replay protection、cross-runtime replay validation は runtime package と research/runtime repository が所有する。 |
