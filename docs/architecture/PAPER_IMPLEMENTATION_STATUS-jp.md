...
| 03 | Pre-Inference Admissibility Governance | **Partially contracted**。PDP、Guard、prompt validation、policy evaluation、fail-closed DTO/enum、critical-operation / computational-complexity gate interface、admissibility replay / profile / budget / result vocabulary（`AdmissibilityGateKind`、`AdmissibilityDecisionKind`、`TaskCostClass`、`CriticalOperationRequirement`、`AdmissibilityReplayRecord`）に反映済み。 | 完全な admission orchestration と deterministic gate implementation は Core/runtime package が所有する。 |
| 04 | Trajectory Governance Model | **Partially contracted**。attention observer/guard、lifecycle、governance stats、failure enum、DynamicSLM trajectory metadata、ReplayLog-compatible thought-artifact record、semantic ellipsoid / convergence・anomaly score report / candidate action evaluation を運ぶ trajectory governance DTO に反映済み。 | semantic trajectory runtime、scoring algorithm、PDP orchestration は AIKernel.NET には未実装であり、Core/runtime package が所有する。 |
...
| 06 | Semantic Compilation Architecture | **Partially contracted**。context、routing、DSL、pipeline、replay 関連 DTO/interface、`SemanticIrSlot`、semantic state / IR element / governed circuit / prototype space / distance report / deterministic synthesis / transition を運ぶ Semantic Compilation DTO に反映済み。 | semantic compiler runtime、prototype-space search、semantic distance evaluation、graph execution は Core/runtime package が所有する。 |
...
| 08 | Operator Architecture | **Partially contracted**。CLI、managed assembly、native ABI、DSL ROM、remote endpoint 境界向けの external Capability module descriptor / invocation envelope と registry / invoker interface に反映済み。 | Lean/C/C# operator export、sandbox execution、assembly loading、native invocation、provider runtime は `AIKernel.RH`、Core/Tools、host/native package が所有する。 |
...
| 12 | Canonical Trajectory Governance (CTG) | **Contracted for v0.1.1.1**。CTG council / gate / trajectory / ROM governance interface、`CouncilKind`、`CouncilVoteValue`、`CouncilDecisionKind`、discrete gate decision enum、`CanonReference`、`RejectReasonInfo`、`CouncilVote`、`CouncilDecision`、`DecisionGateResult`、`StepGovernanceTrace`、`GovernanceTrace`、ROM governance carrier に反映済み。実装は論文 sketch より詳細だが、論文 invariant を維持する。 | finite vote-table execution、Ethos-denial short-circuit、CTG-ROM loading、persona profile resolution、PPM research integration、HATL-backed state/replay protection、cross-runtime replay validation は runtime package と research/runtime repository が所有する。 |

## Current Completion Summary

- Phase-1 の contract foundation は interface / DTO 境界として概ね存在する。
- runtime behavior は意図的にこのリポジトリの外に置いている。
- Paper 03、06、10、11 は、admission evidence、Semantic IR/state/circuit/prototype/distance/synthesis/transition descriptor、DSL compilation、DynamicSLM / SeedSLM discipline の共有 contract vocabulary を公開する。ただし runtime behavior は AIKernel.NET へ移動しない。
- Paper 09 (HATL) は contract foundation を追加済み。ただし cryptographic runtime は外部に残す。
- Paper 12 (CTG) は three-council governance、deterministic gate carrier、CTG-ROM reference、replayable governance trace の contract foundation を追加済み。C# enum numeric value は serialization discriminant であり、論文の数学的 vote weight ではない。
...
- v0.1.1.1 (2026-06-14): Paper 12 CTG mapping と vote-weight alignment note を追加。
