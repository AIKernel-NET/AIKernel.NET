...
| 06 | Semantic Compilation Architecture | **Partially contracted**。context、routing、DSL、pipeline、replay 関連 DTO/interface、`SemanticIrSlot`、semantic state / IR element / governed circuit / transition を運ぶ Semantic Compilation DTO に反映済み。 | semantic compiler runtime、prototype-space search、semantic distance evaluation、graph execution は Core/runtime package が所有する。 |
...
- Paper 03、06、10、11 は、admission evidence、Semantic IR/state/circuit/transition descriptor、DSL compilation、DynamicSLM / SeedSLM discipline の共有 contract vocabulary を公開する。ただし runtime behavior は AIKernel.NET へ移動しない。
