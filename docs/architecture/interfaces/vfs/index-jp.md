...
- Package: これらの contract は `AIKernel.Abstractions` から直接公開する。個別の `AIKernel.Vfs` 互換 package は v0.0.4 で削除された
...
- `AIKernel.Vfs` は公開 namespace であり、個別 package/project dependency ではない。
...
- v0.0.4 (2026-06-04): 公開 namespace を維持したまま個別の AIKernel.Vfs 互換 package を削除
