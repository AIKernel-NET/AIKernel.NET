...
英語版: [Vfs Capability Contracts](../vfs/IVfsCapabilityContracts.md)
...
v0.0.3 以降、これらの contract は公開 namespace `AIKernel.Vfs` を維持したまま、`AIKernel.Abstractions` が所有する。`AIKernel.Vfs` package は type-forwarding 互換 facade として残る。
...
- v0.0.4 (2026-06-04): DSL / History ROM store から利用される Vfs capability contract が安定していることを確認
