...
- `DslNodeTypes` は serialized node name の canonical vocabulary を定義する。`CapabilityCall` や `SuspendForApproval` のような parser alias は互換語彙であり、`CallCapability` と `Suspend` へ正規化する。
