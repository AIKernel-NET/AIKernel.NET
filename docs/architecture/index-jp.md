...
**ファイル:** `5.PREPROCESSING_VS_PROMPTING.md`
...
AIKernel の最終アーキテクチャ目標は **Semantic Context OS** である。
- Forkable Reasoning
- Committee Reasoning
- Signed Prompt Governance (Fail-Closed)
- Three-Layer Buffer Model

この文書は、再現性・統治性・モデル非依存推論を統合するアーキテクチャの北極星を定義する。
...
注記: `16` までが `architecture/` 本編であり、`18` は設計文書（`/design/DESIGN_INTENT`）への参照番号として予約している。
