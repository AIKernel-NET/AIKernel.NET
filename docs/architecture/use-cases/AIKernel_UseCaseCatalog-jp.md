...
**目的**  
ユーザー入力（自然言語または構造化データ）を受理し、`IInputNormalizer` で正規化したうえで、`ContextFragment` として適切なコンテキスト層へ分配する。
...
**目的**  
`IAttentionGuard` を用いて指示・素材・出力を物理的に隔離し、attention 汚染を防止する。
...
**目的**  
検証済みプロンプトに `IPromptSigner` で署名し、信頼済み仕様として確定する。
...
**目的**  
複数モデルの推論結果を統合し、単一モデル依存を避けて意思決定ロバスト性を高める。
...
**目的**  
モデル出力間の主張・根拠・制約矛盾を検出し、fail-closed 再評価へルーティングする。
...
**目的**  
モデル固有の強みを合成し、単独モデルでは到達しない推論深度を実現する。
