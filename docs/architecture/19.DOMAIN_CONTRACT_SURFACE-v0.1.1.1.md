...
the `CouncilVote` carrier and the `CouncilVoteValue` enum, rejection details by
`RejectReasonInfo`, and canon links by `IReadOnlyList<CanonReference>` so that
multiple canon references can be preserved without changing the DTO shape later.
...
[`21.CTG_DEVELOPER_THEORY-v0.1.1.1.md`](21.CTG_DEVELOPER_THEORY-v0.1.1.1.md),
