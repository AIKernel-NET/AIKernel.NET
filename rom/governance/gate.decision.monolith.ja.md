...
Ethosが **Reject** を投じた場合、決定は即座に **Deny** となる。 この規則は他のすべてのロジックを覆す。
...
これらすべての評価は、排他的に各評議会に属する。

意思決定ゲートは、3つの離散的な票に対する純粋関数である。

## 7. 決定論とリプレイ (Determinism and Replay)

意思決定ゲートは、同一の入力に対して同一の結果を生成しなければならない。 すべての決定は、ガバナンス証跡のリプレイを通じて再現可能でなければならない。
