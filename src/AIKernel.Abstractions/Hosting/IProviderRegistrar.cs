...
{
    /// <summary>EN: Executes the Register operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで Register 操作を実行します。</summary>
    void Register(string providerId, Func<IProvider> factory);
    /// <summary>EN: Executes the TryResolve operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで TryResolve 操作を実行します。</summary>
    bool TryResolve(string providerId, out IProvider? provider);
    /// <summary>EN: Executes the GetRegisteredIds operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで GetRegisteredIds 操作を実行します。</summary>
