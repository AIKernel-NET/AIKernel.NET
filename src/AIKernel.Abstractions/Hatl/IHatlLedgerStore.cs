...
/// <summary>[EN] Documents this public package API member. [JA] IHatlLedgerStore contract を定義します。</summary>
...
public interface IHatlLedgerStore
{
    ...
    /// <summary>EN: Executes the ReadAsync operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで ReadAsync 操作を実行します。</summary>
    ValueTask<HatlLedgerEntry?> ReadAsync(
        string ledgerId,
        long sequenceNumber,
        CancellationToken cancellationToken = default);

    /// <summary>EN: Executes the ReadRangeAsync operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで ReadRangeAsync 操作を実行します。</summary>
