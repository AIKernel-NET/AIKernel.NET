...
/// JA: IPipelineOrchestrator の公開契約を定義します。
/// </summary>
...
public interface IPipelineOrchestrator
{
    ...
    /// </summary>
    /// <param name="context">統合コンテキスト JA: context パラメーターです。</param>
    /// <param name="signatureVerificationResult">署名検証結果（SGS-006 通過済みであることを示す） JA: signatureVerificationResult パラメーターです。</param>
    /// <param name="cancellationToken">キャンセルトークン JA: キャンセル通知を監視するトークンです。</param>
    /// <returns>全フェーズメタデータを含む ExecutionResult JA: 結果を返します。</returns>
    ...
    /// JA: ReplayFromDumpAsync 操作を実行します。
    /// </summary>
    /// <param name="replayDump">再実行起点となるダンプ JA: replayDump パラメーターです。</param>
    /// <param name="modificationContext">修正情報コンテキスト JA: modificationContext パラメーターです。</param>
    /// <param name="cancellationToken">キャンセルトークン JA: キャンセル通知を監視するトークンです。</param>
    ...
    /// <param name="cancellationToken">キャンセルトークン JA: キャンセル通知を監視するトークンです。</param>
    /// <returns>初期化が成功したかどうか JA: 結果を返します。</returns>
