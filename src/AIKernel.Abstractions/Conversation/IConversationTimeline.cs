...
/// JA: IConversationTimeline の公開契約を定義します。
/// </summary>
...
{
    /// <summary>Executes the GetTimelineAsync operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで GetTimelineAsync 操作を実行します。</summary>
    Task<IReadOnlyList<IConversationSnapshot>> GetTimelineAsync(string branchId, CancellationToken ct = default);
    /// <summary>Executes the GetHeadAsync operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで GetHeadAsync 操作を実行します。</summary>
    Task<IConversationSnapshot?> GetHeadAsync(string branchId, CancellationToken ct = default);
    /// <summary>Executes the FindCheckpointAsync operation on the AIKernel public contract surface. JA: AIKernel の公開契約サーフェスで FindCheckpointAsync 操作を実行します。</summary>
